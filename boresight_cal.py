...
boresight_cal.py - Boresight drift calibration for GLT holography data

Phase-only calibration using robust spline fit through boresight measurements.

Usage:
    python boresight_cal.py input_raw.txt -o output_cal.txt [options]
    
Modes:
    Default:              Apply phase calibration and optionally remove boresight points
    --no-correction:      Remove boresight points WITHOUT applying phase correction
                          (useful for comparing calibrated vs uncalibrated results)
"""
...
def parse_args():
    ...
    parser.add_argument("--correct-amplitude", action="store_true",
                        help="Also correct amplitude drift (default: phase-only)")
    ...
    parser.add_argument("--no-correction", action="store_true",
                        help="Remove boresight points but do NOT apply phase/amplitude correction")
...
def main():
    ...
    cal_phase = apply_phase_correction(phase, phase_drift, verbose=args.verbose)
    
    # Amplitude: keep original unless --correct-amplitude specified
    if args.correct_amplitude:
        if args.verbose:
            print("Applying amplitude correction...")
        ref_amp = bore_amps[0]
        amp_drift_bore = bore_amps - ref_amp
        amp_drift = np.interp(timestamps, bore_times, amp_drift_bore)
        cal_amplitude = amplitude - amp_drift
    else:
        cal_amplitude = amplitude
    ...
    if args.correct_amplitude:
        print("  Amplitude drift corrected: {:.6f}".format(
            amp_drift.max() - amp_drift.min()))
    else:
        print("  Amplitude: unchanged (phase-only correction)")
    ...
    if args.plot:
        try:
            ...
            ax.plot(timestamps[::step], phase_drift[::step], 'b-', lw=1, alpha=0.7)
            ax.set_xlabel('Time (seconds)')
            ax.set_ylabel('Phase correction (degrees)')
            ax.set_title('Applied Phase Drift Correction ({})'.format(method_name))
            ax.grid(True, alpha=0.3)
            
            plt.tight_layout()
            plot_filename = f"{prefix}_boresight_cal_diagnostic.png"
