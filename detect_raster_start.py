...
detect_raster_start.py — Interactive raster start detection
...
Features:
  ...
  ✓ Output file: preserves timestamps if present (for boresight calibration)
    columns = timestamp, az, el, amp, phase  (or az, el, amp, phase if no timestamp)
  ✓ Inspection plots (3-panel) commented out - moved to regridding program

Updated May 2026: Added --keep-timestamps option to preserve timestamps for
boresight drift calibration pipeline.
"""
...
def parse_timestamp(ts_str):
    ...
    - Decimal seconds of day: 50099.601255
    - HH:MM:SS format: 12:59:07
    """
...
def read_ascii(path: str, has_header: bool = False, comment: str = "#") -> Tuple[pd.DataFrame, List[str], bool]:
    """
    Read ASCII holography data file.
    
    Returns:
        df: DataFrame with columns [timestamp, az, el, amp, phase] or [az, el, amp, phase]
        header_lines: list of header/comment lines
        has_timestamp: True if data has 5 columns (timestamp present)
    """
    header_lines, rows = [], []
    with open(path, "r", encoding="utf-8", errors="ignore") as f:
        for line in f:
            stripped = line.strip()
            if stripped.startswith(comment) or stripped == "":
    ...
    # Detect number of columns from first data row
    first_row_parts = rows[0].split()
    n_cols = len(first_row_parts)
    
    tmp = "".join(rows)
    if n_cols >= 5:
        # Has timestamp: timestamp, az, el, amp, phase
        has_timestamp = True
        if has_header:
            df = pd.read_csv(pd.io.common.StringIO(tmp), sep=r'\s+', comment=comment)
        else:
            df = pd.read_csv(pd.io.common.StringIO(tmp), sep=r'\s+', header=None,
                             names=["timestamp", "az", "el", "amp", "phase"])
        # Convert timestamp to numeric (handles both formats)
        df["timestamp"] = df["timestamp"].apply(parse_timestamp)
    else:
        # No timestamp: az, el, amp, phase
        has_timestamp = False
        if has_header:
            df = pd.read_csv(pd.io.common.StringIO(tmp), sep=r'\s+', comment=comment)
        else:
            df = pd.read_csv(pd.io.common.StringIO(tmp), sep=r'\s+', header=None,
                             names=["az", "el", "amp", "phase"])
    
    # Convert numeric columns
    for c in ["timestamp", "az", "el", "amp", "phase"]:
        if c in df.columns:
            df[c] = pd.to_numeric(df[c], errors="coerce")
    
    return df.dropna().reset_index(drop=True), header_lines, has_timestamp
...
def main():
    ...
    ap.add_argument("--keep-timestamps", action="store_true",
                    help="Preserve timestamps in output (required for boresight calibration)")
    ap.add_argument("--no-plot", action="store_true",
                    help="Skip interactive plots and auto-accept detected start (faster over slow network)")
    # New options for GUI support
    ap.add_argument("--detect-only", action="store_true",
                    help="Only detect start line, print 'DETECTED_START: N' and exit (for GUI)")
    ap.add_argument("--preview-plot", type=int, metavar="LINE",
                    help="Show trajectory plot with LINE marked, then exit (for GUI)")
    ap.add_argument("--start-line", type=int, metavar="LINE",
                    help="Skip detection, use LINE as start (for GUI after user confirmation)")
    args = ap.parse_args()

    df, header_lines, has_timestamp = read_ascii(args.input, has_header=args.assume_header)

    if args.keep_timestamps and not has_timestamp:
        print("Warning: --keep-timestamps specified but input file has no timestamps")
    
    if has_timestamp:
        print(f"Input file has timestamps (will {'preserve' if args.keep_timestamps else 'discard'} them)")

    # Handle --preview-plot: just show plot and exit
    if args.preview_plot is not None:
        print(f"Showing preview plot with start line = {args.preview_plot}")
        fig, _ = plot_trajectory_dual(df, args.preview_plot, "red", f"Start line {args.preview_plot}")
        plt.show(block=True)
        sys.exit(0)

    # Determine start_idx
    if args.start_line is not None:
        # Use user-specified start line
        start_idx = args.start_line
        print(f"\nUsing specified start line = {start_idx}")
    else:
        # Auto-detect start
    ...
    # Handle --detect-only: print and exit
    if args.detect_only:
        print(f"DETECTED_START: {start_idx}")
        sys.exit(0)
    
    if args.no_plot:
        # Skip all plotting, auto-accept detected start
        print("--no-plot specified: auto-accepting detected start point")
    else:
    ...
    if args.keep_timestamps and has_timestamp:
        output_cols = ["timestamp", "az", "el", "amp", "phase"]
        fmt = "%.6f %.6f %.6f %.9g %.9g"
    else:
        output_cols = ["az", "el", "amp", "phase"]
        fmt = "%.9g"
    
    # Fast file writing using numpy
    print(f"\nWriting trimmed file with {len(trimmed)} data points...")
    trimmed_array = trimmed[output_cols].values
    np.savetxt(out_path, trimmed_array, fmt=fmt, delimiter=" ")
    print(f"Wrote trimmed file: {out_path}")
    if args.keep_timestamps and has_timestamp:
        print(f"  Format: timestamp az el amp phase")
    else:
        print(f"  Format: az el amp phase")
