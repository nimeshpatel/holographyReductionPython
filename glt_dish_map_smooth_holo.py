...
def main():
    ...
    parser.add_argument('--holo-smooth', nargs='?', const='hanning',
                        choices=['nearest', 'none', 'bilinear', 'bicubic', 'hanning', 'lanczos', 'spline16', 'spline36'],
                        default='hanning',
                        help='Holography display interpolation. Default hanning gives a smoother unwrap2d-like display; use nearest/none for square pixels.')
    ...
    if is_grid_data:
        ...
        holo_interp = 'nearest' if args.holo_smooth in (None, 'none') else args.holo_smooth
        im = ax.imshow(error_grid, extent=extent, origin='upper', cmap=cmap, aspect='equal',
                       vmin=args.vmin, vmax=args.vmax,
                       interpolation=holo_interp,
                       filternorm=args.holo_filternorm,
                       filterrad=args.holo_filterrad,
                       resample=True)
    else:
        xp = xtarg
        yp = ytarg
