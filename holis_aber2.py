...
# Configure logging - mode is set dynamically in main() once prefix is known
# (FileHandler added there so we can use write mode and record the prefix)
logging.basicConfig(
    level=logging.INFO,
    format='%(message)s',
    handlers=[
...
Examples:
  ...
  %(prog)s -a 1 -u             # Explicit algorithm 1 with unwrap
    ...
    parser.add_argument('-p', '--prefix', type=str, default=None,
                        help='Dataset prefix (e.g. holoADC-AzEl-20260515_165219). '
                             'Recorded in holis.log and used for fit JSON sidecar name.')
...
def post_process(c: np.ndarray, params: dict, ndim: int, ncent: int,
    ...
    ph, ph_um, chisq, coef = holis_fit.phasefit_aber2(
    ...
    # Return fit results for JSON sidecar
    fit_results = {
        'rms_before_rad':  round(rms_before_rad, 8),
        'rms_after_rad':   round(float(rms1), 8),
        'rms_after_um':    round(float(rms1_microns), 4),
        'chisq':           round(float(chisq), 6),
        'freq_ghz':        params['freq'],
        'wavelength_mm':   round(alambda * 1e3, 6),
        'dofit':           [int(x) for x in params['dofit']],
        'coef': {
            'DC_offset_rad':      round(float(coef[0]), 8),
            'tilt_x_rad_pix':     round(float(coef[1]), 8),
            'tilt_y_rad_pix':     round(float(coef[2]), 8),
            'defocus_mm':         round(float(coef[3] * 1000), 6),
            'astigmatism_45_deg': round(float(coef[4]), 8),
            'astigmatism_0_deg':  round(float(coef[5]), 8),
            'coma_x':             round(float(coef[6]), 8),
            'coma_y':             round(float(coef[7]), 8),
        },
        'unwrapped': use_unwrap,
    }
    return fit_results
...
def main():
    ...
    # Now we know the prefix — attach the FileHandler in WRITE mode
    # so each run starts a fresh holis.log (no unbounded accumulation).
    fh = logging.FileHandler('holis.log', mode='w')
    fh.setFormatter(logging.Formatter('%(message)s'))
    logger.addHandler(fh)

    # Record the run header so the log is useful later
    logger.info("="*70)
    logger.info("HOLIS_ABER2 - Holography Aperture Field Calculation")
    logger.info(f"Run at: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    if args.prefix:
        logger.info(f"Dataset prefix: {args.prefix}")
    logger.info("="*70)
    logger.info("")
    ...
    if args.algorithm == 1:
        ...
        logger.info("")
        fit_results = run_phase_coherent(args.unwrap)
    ...
    # Write fit results to JSON sidecar (holis_fit.json in working dir).
    # holoMap.sh copies this to results/${PREFIX}_fit.json alongside the log.
    if fit_results is not None:
        from datetime import datetime as _dt
        fit_results['prefix'] = args.prefix or ''
        fit_results['run_time'] = _dt.now().strftime('%Y-%m-%d %H:%M:%S')
        with open('holis_fit.json', 'w') as _f:
            json.dump(fit_results, _f, indent=2)
        logger.info(f"Fit results written to holis_fit.json")
...
def run_phase_coherent(use_unwrap: bool = False):
    ...
    return post_process(c, params, ndim, ncent, ndish, nsub, pixbylen, use_unwrap)
