...
cpy3 detect_raster_start.py $1 -o trimmed.txt

echo "Regridding data to a 128 x 128 map..."
cpy3 regrid_holo.py trimmed.txt regrid_128x128_29oct25.prm
...
cpy3 glt_dish_map.py results/$PREFIX.Ea_um.dat --x-shift -65 --y-shift 65
cpy3 glt_dish_map.py results/$PREFIX.Ea_um.dat --x-shift -65 --y-shift 65 --output results/${PREFIX}_illumination.pdf

echo "Generating phase residual map..."
cpy3 glt_dish_map.py results/${PREFIX}_Epr.dat --vmin -180 --vmax 180 --x-shift -65 --y-shift 65
cpy3 glt_dish_map.py results/${PREFIX}_Epr.dat --vmin -180 --vmax 180 --x-shift -65 --y-shift 65 --output results/$PREFIX.pdf
...
#    cpy3 create_summary_pdf.py "$1" "results/${PREFIX}"
#else
#    cpy3 create_summary_pdf.py "$1" "results/${PREFIX}" --comment "$user_comment"
#fi
#
#echo ""
