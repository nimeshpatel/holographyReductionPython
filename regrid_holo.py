...
def plot_regridded_data(outfile: Path, params: dict):
    ...
    # ── Joint-plot layout ─────────────────────────────────────────────────
    # 2×2 GridSpec:
    #   [0,0]  az beam     (az vs amp)       [0,1]  empty / corner
    #   [1,0]  square map  (az vs el)        [1,1]  el beam (amp vs el, rotated)
    #
    # The az beam sits above the map (shared x-axis).
    # The el beam sits to the right of the map (shared y-axis).
    # ─────────────────────────────────────────────────────────────────────

    az_min, az_max = az.min(), az.max()
    el_min, el_max = el.min(), el.max()

    map_size  = 5.5    # inches — side length of the square map panel
    beam_size = 1.8    # inches — height/width of each beam panel
    fig_w = map_size + beam_size + 1.2
    fig_h = map_size + beam_size + 1.0

    fig = plt.figure(figsize=(fig_w, fig_h))

    gs = fig.add_gridspec(
        2, 2,
        width_ratios  = [map_size, beam_size],
        height_ratios = [beam_size, map_size],
        hspace = 0.04,
        wspace = 0.04,
        left   = 0.10,
        right  = 0.97,
    ...
    ax_azb  = fig.add_subplot(gs[0, 0])   # azimuth beam (top)
    ax_corn = fig.add_subplot(gs[0, 1])   # corner — blank
    ax_map  = fig.add_subplot(gs[1, 0])   # main square map
    ax_elb  = fig.add_subplot(gs[1, 1])   # elevation beam (right, rotated 90°)
    ax_corn.axis('off')

    # ── Azimuth beam: amp vs az (top) ─────────────────────────────────────
    az_sort = np.argsort(az)
    az_s, amp_az = az[az_sort], amp[az_sort]
    ax_azb.scatter(az_s, amp_az, s=1.5, c='forestgreen', linewidths=0)
    ax_azb.set_xlim(az_min, az_max)
    ax_azb.set_ylabel("Amplitude", fontsize=9)
    ax_azb.tick_params(labelbottom=False, labelsize=8)
    ax_azb.grid(True, alpha=0.25, linewidth=0.5)

    # ── Map panel: az vs el, tiniest possible markers ─────────────────────
    ax_map.scatter(az, el, s=0.5, c='royalblue', linewidths=0)
    ax_map.set_xlim(az_min, az_max)
    ax_map.set_ylim(el_min, el_max)
    ax_map.set_xlabel("Azimuth (degrees)", fontsize=9)
    ax_map.set_ylabel("Elevation (degrees)", fontsize=9)
#    ax_map.set_title("Regridded Map", fontsize=10, pad=4)
    ax_map.grid(True, alpha=0.25, linewidth=0.5)
    ax_map.tick_params(labelsize=8)

    # ── Elevation beam: amp vs el, rotated 90° clockwise ──────────────────
    # x-axis = amplitude, y-axis = elevation (matches map y-axis)
    el_sort = np.argsort(el)
    el_s, amp_el = el[el_sort], amp[el_sort]
    ax_elb.scatter(amp_el, el_s, s=1.5, c='firebrick', linewidths=0)
    ax_elb.set_ylim(el_min, el_max)
    ax_elb.set_xlabel("Amplitude", fontsize=9)
    ax_elb.yaxis.set_label_position('right')
    ax_elb.yaxis.tick_right()
    ax_elb.tick_params(labelleft=False, labelsize=8)
    ax_elb.grid(True, alpha=0.25, linewidth=0.5)

    plt.savefig('regridded_map.png', dpi=150, bbox_inches='tight')
    plt.show()
